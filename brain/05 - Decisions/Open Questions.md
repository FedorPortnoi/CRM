...
| A6 | Pipeline and Stage — separate Prisma models or just UUIDs on Deal? | **Sprint 2** | **High** | Deal.pipeline_id and Deal.stage_id exist as `String? @db.Uuid` but the Pipeline and Stage tables are not in schema.prisma. The deals route already has `/pipelines` and `/stages` endpoints (stubs). Need to decide: (1) add Pipeline + Stage models to schema.prisma + migration, or (2) keep pipeline management in a separate service/table outside Prisma scope. Recommendation: add to schema — Prisma models give type safety and joins. |
