...
- Inbound call from an unknown number, or a forwarded email from an unknown address (not in contacts): saved to a "Unknown" inbox; user can match to existing contact or create new contact from it
...
- Idempotency: every auto-captured event has a `source_id` (e.g., Google Calendar event ID, inbound email Message-ID); unique constraint on `(organization_id, source_id)` prevents duplicate inserts
