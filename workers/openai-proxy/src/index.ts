...
// PROXY_TOKEN, a separate shared secret that grants nothing but this one
// rate-limited endpoint.
//
// PRIVACY — READ BEFORE ADDING ANY console.log:
// Request and response bodies carry customer contact names, deal titles and
// free-text notes. They are NEVER logged, never stored, and never buffered any
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const finish = (response: Response, fields: LogFields): Response => {
      emitLog(fields);
      return response;
    };

    // --- 1. Routing ------------------------------------------------------
    // Exactly one method + path. Everything else is 404, including the right
    // path with the wrong method: there is no wildcard passthrough, because a
    // general-purpose proxy in front of a funded OpenAI account is an open
    // relay for anyone who ever sees PROXY_TOKEN.
    const url = new URL(request.url);
    if (request.method !== ROUTE_METHOD || url.pathname !== ROUTE_PATH) {
    ...
    // --- 9. Upstream ------------------------------------------------------
    // Headers are built from scratch rather than forwarded. Anything the
    // caller sent — a second Authorization header, OpenAI-Organization, an
    // injected Host — is discarded, so the caller cannot smuggle fields into
    // the upstream request or bill a different account.
...
/**
 * Reads the body while enforcing a hard byte cap.
 *
